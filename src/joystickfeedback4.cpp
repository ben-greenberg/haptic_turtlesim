    ...
    if(init_falcon(falcon_int))
    {
        ...
        while(node.ok())
        {
            ...
            if(m_falconDevice.runIOLoop())
            {
				...
				if(CurPose.y == 0) //hit the bottom of the window 
				{
					ROS_WARN("I hit the bottom");
					if((CurPose.theta >= 0 && CurPose.theta <= 3.14) || (CurPose.theta <= -3.14)) //facing up
					{
						forces[1] = 10;		//move forward
						ROS_INFO("move forward");
					}
					else
					{
						forces[1] = -10;	//move backward
						ROS_INFO("move backward");
					}	
				}
				
				if(CurPose.y >= 11) //hit the top of the window 
				{
					ROS_WARN("I hit the top");
					if((CurPose.theta >= 0 && CurPose.theta <= 3.14) || ( CurPose.theta <= -3.14)) //facing up
					{
						forces[1] = -10;	//move backward
						ROS_INFO("move backward");
				...
				if(CurPose.x == 0) //hit the left of the window 
				{
					ROS_WARN("I hit the left");
					if((CurPose.theta >= -1.07 && CurPose.theta <= 1.07) || (CurPose.theta >= 4.21)) //facing right
					{
						forces[1] = 10;		//move forward
						ROS_INFO("move forward");
					}
				...
				if(CurPose.x >= 11) //hit the right of the window 
				{
					ROS_WARN("I hit the right");
					if((CurPose.theta >= -1.07 && CurPose.theta <= 1.07) || (CurPose.theta >= 4.21)) //facing right
					{
						forces[1] = -10;	//move backward
						ROS_INFO("move backward");
